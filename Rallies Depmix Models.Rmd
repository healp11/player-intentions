...
fcov1_df <-  as.data.frame(
  c(logLik(fcov1$p.start.position.x), logLik(fcov1$p.start.position.y),
    ...
    logLik(fcov1$rally.side.change.count.diff), logLik(fcov1$p.diff.avg.shot.and.match.movement.speed),
    logLik(fcov1$oppo.diff.avg.shot.and.match.movement.speed),
    logLik(fcov1$diff.p.avg.and.current.shot.speed),
    logLik(fcov1$lag.diff.oppo.avg.and.current.shot.speed)  )
  )
...
fed16_plot1<- fed16_scale %>%
  gather(key = vars, value = measurement, p.start.position.x, oppo.start.position.x, rally.side.change.count.diff)
