...
fed16_df <- fed16_df %>% rowwise %>%
  mutate(end.server.x = ifelse(server == "FEDERER" & end.receiver.x >= 0, -end.server.x, end.server.x)) %>%
  mutate(end.server.y = ifelse(server == "FEDERER" & end.receiver.x >= 0, -end.server.y, end.server.y)) %>%
...
```{r}
ggplot(fed_only, aes(projected.ballmark.x, projected.ballmark.y)) + geom_point()
```
