...
```{r}
fed16_longest <- filter(longral_pos, rally.number=="73")
...
fed_scale <- fed_scale %>%
  mutate(winner.return.error = ifelse(winner.return.error == 1,1,
                                      ifelse(winner.return.error == 0.5,0,-1)))
...
#Saving all .csv files needed to run the report without all the code above

```{r}
write.csv(stry_scale, "data/stry_scale.csv")
write.csv(longral_pos,"data/longral_pos.csv")
write.csv(fed16_longest,"data/fed16_longest.csv")
write.csv(fed16_only, "data/fed16_only.csv")
write.csv(fed16_scale, "data/fed16_scale.csv")
write.csv(fed16_df, "data/fed16_df.csv")
write.csv(fed17_only, "data/fed17_only.csv")
write.csv(fed18_only, "data/fed18_only.csv")
write.csv(fed_scale, "data/fed_scale.csv")
write.csv(stry16_only, "data/stry16_only.csv")
write.csv(stry17_only, "data/stry17_only.csv")
write.csv(stry_scale, "data/stry_scale.csv")
write.csv(fed16_only_plots, "data/fed16_only_plots.csv")
write.csv(dlb_fedonly, "data/dlb_fedonly.csv")
write.csv(iof_fedonly, "data/iof_fedonly.csv")
write.csv(dlw_fedonly, "data/dlw_fedonly.csv")
write.csv(longral_df, "data/longral_df.csv")
write.csv(fed16_pos, "data/fed16_pos.csv")
...
#Modelling
Visual analysis has been used to select a number of new variables from the new data set. For the two hdden state model 'winner' is the response variable. For the three hidden state model 'winner.return.error' is the response variable. 
...
##Training Models
For Stepwise, begin the model running all the covariates to find the covariate which lowers the log Likelihood of the model the most. Then add this covariate to the model and run all the remaining covariates to find the next best covariate which improves the model the most. Repeat this until the model is no longer being improved through the addition of new covariates or until there are no more covariates.  
...
##2 state Model
The best two state model by comparing total change in -logLik is running Training Model 2 the match data Federer vs Berdych 2016/17/18.
...
Intepreting the coeficcients of the model suggests that. 
...
An increases oppo.start.position.x decreases the probablity of hitting a winner. As Berdych starts further away from the net Federer is more likely to hit a winner. 
...
An Increase in p.diff.avg.shot.and.match.movement.speed decreases the probability of hitting a winner. If Federer is runningfaster to hit the shot he is lesss likely to hit a winner. 

An increase in lag.oppo.height.off.net increases the probability of hitting a winner. If Berdych's shot is coming higher over the net then Federer is more likely to hit a winner.  
...
Unforrtunately there were some issues with my models. 
Repeated running of the depmix models  in this report can see the attacking state alternate between being defined as state 1 or state 2. The results remain the same for the probability of hitting a winner, but the corresponding states are flipped. 
We have to be careful in intepreting the results. The attacking state is the one which has a higher value for Re1.1. 
...
I then ran the three state models with the player's advantage variable as the second response variable. At the zero values of the covariates, three clear states can be seen. However with repeated running of these models, the results are not repeatable.

The three state model would be useful for identifying the probability of the player hitting a winner or an error from an attacking, returning or a defenisve state. A larger probability of hitting a winner from a defensive state compared to other players, may be an indicator of player risk taking. 
...
We can take the implied states from the Depmix Model probs and overlay them into the animation to visualise when the player is in an attacking state. In this casewe are looking at the longest rally between Federer and Berdych in 2016. Federer will get larger to indicate when he is an attacking state.
...
```{r}
#For the simplicity of the example I have taken the state probabilities from the Depmix model for Federer vs Berdych 2016 and created a vector for the longest rally
...
commented out to reduce knit time
...
```{r}
ggplot(stry16_only, aes(x=winner.return.error, y=p.advantage)) + geom_bar(stat="identity") + xlab("winner Return Error") + ylab("Player Advantage") + ggtitle("Winners By Number of Times Federer Changes Side in Rally")
...
We can also split the animation by rallies here is some code for that which has commented out for the report. 
