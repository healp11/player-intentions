...
```{r}
p.rally.anim2 <- ggplot(longest_df, aes(x=p.start.position.x, xend=p.end.position.x, y=p.start.position.y, yend=p.end.position.y, group = rally.number)) + 
  geom_point() +
  geom_path(aes(alpha = shot)) +
