...
trial_df <- filter(fed16_pos, final.shot >= 14)
 ...
 p.rally.paths <- ggplot(trial_df, aes(x=p.start.position.x, xend=p.end.position.x, y=p.start.position.y, yend=p.end.position.y))    + geom_point()  + geom_path(arrow = arrow(angle = 15)) +  facet_wrap(~rally.number) + xlab("Federer Position X ") + ylab("Federer Position Y") + ggtitle("Federer start and end positions for rallies 14 shots or longer") 
...
p.rally.anim <- ggplot(longest_df, aes(x=p.start.position.x, xend=p.end.position.x, y=p.start.position.y, yend=p.end.position.y)) + 
  geom_point() + xlab("Federer Position X ") + 
  ylab("Federer Position Y") + 
  ggtitle("Federer start and end positions for rallies 14 shots or longer") +
  transition_states(location.id, transition_length = 4, state_length = 2)

animate(p.rally.anim)
```
