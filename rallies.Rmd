...
Federer appears more likely to hit a winner as Berdych start position x coordinates are closer to the net. (As we are analysing the rallies with all of Berdychs having positive coordinates in the HMM we would expect a decrease in Berdychs start position x coordinates to increase The probability of Federer being in an attaccking state. 
...
ggplot(fed16_only_plots, aes(x=factor(winner),y=abs(p.diff.avg.shot.and.match.movement.speed), fill=factor(winner))) + geom_violin(scale = "area") + ggtitle("Winner by absolute difference in Federer's current/match avg movement speed")
...
ggplot(fed16_only, aes(x=factor(winner),y=p.movement.angle.1, fill=factor(winner.return.error))) + geom_violin(scale = "area") + ggtitle("Winner by Federer's start to end movement angle")
