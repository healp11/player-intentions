...
ggplot(longral_pos, aes(x=shot,y=avg.player.speed.diff)) +
  ...
  ggtitle("Federer vs Opponent difference Avg Movement Speed m/s  Federer and Opponent in rallies 10 shots or more")
...
library(depmixS4)
library(dplyr)
t1.mod1 <- depmix(winner ~ 1, transition = ~ start.x + start.y +p.start.position.x + p.start.position.y +  projected.ballmark.x + p.start.position.base.diff + p.end.position.base.diff + avg.player.speed.diff + p.start.position.centre.diff + p.end.position.centre.diff,  data = fed_only, nstates = 2, family=multinomial("identity"))
t1.fm1 <- fit(t1.mod1)
summary(t1.fm1)
```
...
```{r}
t1.mod2 <- depmix(winner ~ 1, transition = ~ start.x + start.y +p.start.position.x + p.start.position.y,  data = fed_only, nstates = 2, family=multinomial("identity"))
t1.fm2 <- fit(t1.mod2)
summary(t1.fm2)
```
