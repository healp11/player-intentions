...
```
```{r}
thirtyfour.df <- filter(longral_pos, rally.number=="34")
...
p.rally.anim9 <- ggplot() + 
  scale_x_continuous(breaks = axis_labels$x.break,
    ...
    geom_path(data = net_trace, aes(x = x, y = y), color = 'grey40', size = 1, lineend = 'round') +
    coord_fixed() +
  geom_point(data= thirtyfour.df, aes(x=p.start.position.x, y=p.start.position.y,  group = rally.number, colour="red")) +
  geom_path(data= thirtyfour.df, aes(x=p.start.position.x, y=p.start.position.y,  group = rally.number, alpha = shot)) +
  geom_point(data= thirtyfour.df, aes(x=oppo.start.position.x, y=oppo.start.position.y,  group = rally.number, colour="blue")) +
  geom_path(data= thirtyfour.df, aes(x=oppo.start.position.x, y=oppo.start.position.y,  group = rally.number, alpha = shot)) +
  geom_point(data= thirtyfour.df, aes(x=start.x, y=start.y, group = rally.number, colour="green")) +
  transition_reveal(pos.rally.count) +
  xlab("Player Position X ") + 
  ylab("Player Position Y") + 
  ggtitle("Longest Rally Animation")+
