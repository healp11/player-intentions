...
inside_func <- function(data, x){
  depmix(winner ~ 1, data = data, transition = ~ x, nstates = 2, family=multinomial("identity"))
  }

# for every possible variable in data set
Loop_trial <- ha %>% map(~ 
  )
   t6.mod41 <- depmix(winner ~ 1, data = fed_only, transition = ~ cova, nstates = 2, family=multinomial("identity"))
