...
fed16_df <- fed16_df %>%
  mutate(lag.p.end.position.x=NA) %>%
  mutate(lag.p.end.position.y=NA) %>%
  mutate(lag.oppo.end.position.x=NA) %>%
  mutate(lag.oppo.end.position.y=NA)

for (i in 2:nrow(fed16_df)) {
  fed16_df$lag.p.end.position.x[i]=fed16_df$p.end.position.x[i-1]
  fed16_df$lag.p.end.position.y[i]=fed16_df$p.end.position.y[i-1]
  fed16_df$lag.oppo.end.position.x[i]=fed16_df$oppo.end.position.x[i-1]
  fed16_df$lag.oppo.end.position.y[i]=fed16_df$oppo.end.position.y[i-1]
}


insert_df <- transform(fed16_df, p.start.position.x=lag.p.end.position.x, p.end.position.x=p.start.position.x,p.start.position.y=lag.p.end.position.y, p.end.position.y=p.start.position.y, oppo.start.position.x=lag.oppo.end.position.x, oppo.end.position.x=oppo.start.position.x,oppo.start.position.y=lag.oppo.end.position.y, oppo.end.position.y=oppo.start.position.y, start.x=projected.ballmark.x, start.y=projected.ballmark.y)

fed16_pos <- bind_rows(fed16_df, setNames(insert_df, names(fed16_df))) %>% 
...
trial_df <- filter(fed16_pos, final.shot >= 14)
 ...
 p.rally.paths <- ggplot(trial_df, aes(x=p.start.position.x, xend=p.end.position.x, y=p.start.position.y, yend=p.end.position.y))    + geom_point(aes(alpha = shot))  + geom_path(  arrow = arrow(angle = 15)) +  facet_wrap(~rally.number) + xlab("Federer Position X ") + ylab("Federer Position Y") + ggtitle("Federer start and end positions for rallies 14 shots or longer") 
...
oppo.rally.paths <- ggplot(trial_df, aes(x=oppo.start.position.x, xend=oppo.end.position.x, y=oppo.start.position.y, yend=oppo.end.position.y))    + geom_point()  + geom_segment(arrow = arrow(angle = 15)) +  facet_wrap(~rally.number) + xlab("Opponent Position X ") + ylab("Opponent Position Y") + ggtitle("Opponent start and end positions for for rallies 14 shots or longer")  
...
p.rally.anim2 <- ggplot() + 
  scale_x_continuous(breaks = axis_labels$x.break,
    ...
    geom_path(data = net_trace, aes(x = x, y = y), color = 'grey40', size = 1, lineend = 'round') +
    coord_fixed() +
  geom_point(data= longest_df, aes(x=p.start.position.x, y=p.start.position.y,  group = rally.number)) +
  geom_path(data= longest_df, aes(x=p.start.position.x, y=p.start.position.y,  group = rally.number, alpha = shot)) +
...
animate(p.rally.anim2, duration = 30, fps = 10)
```
```{r}
p.rally.anim3 <- ggplot() + 
  scale_x_continuous(breaks = axis_labels$x.break,
                       labels = axis_labels$x.label) +
    scale_y_continuous(breaks = axis_labels$y.break,
                       labels = axis_labels$y.label) +
    ...
    geom_path(data = net_trace, aes(x = x, y = y), color = 'grey40', size = 1, lineend = 'round') +
    coord_fixed() +
  geom_point(data= longral_df, aes(x=p.start.position.x, y=p.start.position.y,  group = rally.number)) +
  geom_path(data= longral_df, aes(x=p.start.position.x, y=p.start.position.y,  group = rally.number, alpha = shot)) +
  transition_reveal(pos.rally.count) +
  facet_wrap(~rally.number) +
  xlab("Federer Position X ") + 
  ylab("Federer Position Y") + 
  ggtitle("Federer start and end positions animated for rallies 10 shots or greater") 


animate(p.rally.anim3, duration = 30, fps = 10)
```
```{r}
p.rally.anim4 <- ggplot() + 
  scale_x_continuous(breaks = axis_labels$x.break,
                       labels = axis_labels$x.label) +
    scale_y_continuous(breaks = axis_labels$y.break,
                       labels = axis_labels$y.label) +
    ...
    geom_path(data = net_trace, aes(x = x, y = y), color = 'grey40', size = 1, lineend = 'round') +
    coord_fixed() +
  geom_point(data= longest_df, aes(x=p.start.position.x, y=p.start.position.y,  group = rally.number, colour="red")) +
  geom_path(data= longest_df, aes(x=p.start.position.x, y=p.start.position.y,  group = rally.number, alpha = shot)) +
  geom_point(data= longest_df, aes(x=oppo.start.position.x, y=oppo.start.position.y,  group = rally.number, colour="blue")) +
  geom_path(data= longest_df, aes(x=oppo.start.position.x, y=oppo.start.position.y,  group = rally.number, alpha = shot)) +
  transition_reveal(pos.rally.count) +
  xlab("Player Position X ") + 
  ylab("Player Position Y") + 
  ggtitle("Federer and opposition start and end positions animated for longest rally")+
  scale_colour_manual(name = "", values=c("red","blue"), labels= c("Berdych","Federer"))

#had to reverse geom_point colour labels for some reason

animate(p.rally.anim4, duration = 30, fps = 10)
```
```{r}
p.rally.anim5 <- ggplot() + 
  scale_x_continuous(breaks = axis_labels$x.break,
                       labels = axis_labels$x.label) +
    scale_y_continuous(breaks = axis_labels$y.break,
                       labels = axis_labels$y.label) +
    ...
    geom_path(data = net_trace, aes(x = x, y = y), color = 'grey40', size = 1, lineend = 'round') +
    coord_fixed() +
  geom_point(data= longral_df, aes(x=p.start.position.x, y=p.start.position.y,  group = rally.number, colour="red")) +
  geom_path(data= longral_df, aes(x=p.start.position.x, y=p.start.position.y,  group = rally.number, alpha = shot)) +
  geom_point(data= longral_df, aes(x=oppo.start.position.x, y=oppo.start.position.y,  group = rally.number, colour="blue")) +
  geom_path(data= longral_df, aes(x=oppo.start.position.x, y=oppo.start.position.y,  group = rally.number, alpha = shot)) +
  transition_reveal(pos.rally.count) +
  facet_wrap(~rally.number) +
  xlab("Player Position X ") + 
  ylab("Player Position Y") + 
  ggtitle("Federer and opposition start and end positions animated for rallies 10 shots or longer")+
  scale_colour_manual(name = "", values=c("red","blue"), labels= c("Berdych","Federer"))

#had to reverse geom_point colour labels for some reason

animate(p.rally.anim5, duration = 30, fps = 10)
```

```{r}
p.rally.anim6 <- ggplot() + 
  scale_x_continuous(breaks = axis_labels$x.break,
                       labels = axis_labels$x.label) +
    scale_y_continuous(breaks = axis_labels$y.break,
                       labels = axis_labels$y.label) +
  ...
  geom_point(data= longral_df, aes(x=oppo.start.position.x, y=oppo.start.position.y,  group = rally.number, colour="blue")) +
  geom_path(data= longral_df, aes(x=oppo.start.position.x, y=oppo.start.position.y,  group = rally.number, alpha = shot)) +
  geom_point(data= longral_df, aes(x=start.x, y=start.y,  group = rally.number, colour="green")) +
  transition_reveal(pos.rally.count) +
  facet_wrap(~rally.number) +
  xlab("Player Position X ") + 
  ylab("Player Position Y") + 
  ggtitle("Federer and opposition positions animated for rallies 10 shots or longer")+
  scale_colour_manual(name = "", values=c("red","green","blue"), labels= c("Berdych","Ball","Federer"))

#had to reverse geom_point colour labels for some reason

animate(p.rally.anim6, duration = 30, fps = 10)
```
```{r}
p.rally.anim7 <- ggplot() + 
  scale_x_continuous(breaks = axis_labels$x.break,
                       labels = axis_labels$x.label) +
    scale_y_continuous(breaks = axis_labels$y.break,
                       labels = axis_labels$y.label) +
  ...
  geom_path(data= longest_df, aes(x=oppo.start.position.x, y=oppo.start.position.y,  group = rally.number, alpha = shot)) +
  geom_point(data= longral_df, aes(x=start.x, y=start.y, group = rally.number, colour="green")) +
  geom_path(data= longest_df, aes(x=start.x, y=start.y,  group = rally.number, colour = "white")) +
  transition_reveal(pos.rally.count) +
  xlab("Player Position X ") + 
  ylab("Player Position Y") + 
  ggtitle("Longest Rally Animation")+
  scale_colour_manual(name = "", values=c("red","green","blue","white"), labels= c("Berdych","Ball","Federer",""))

#had to reverse geom_point colour labels for some reason

animate(p.rally.anim7, duration = 30, fps = 10)
```
...
```{r}
Xpoint1_df <- fed16_df %>% 
  filter(rally.number == 1, shot == 3) %>%
  select(p.start.position.x,  oppo.start.position.x,  start.x ) %>%
  gather( XPosition,  X ) 
head(point1)
Ypoint1_df<- fed16_df %>% 
  filter(rally.number == 1, shot == 3) %>%
  select(p.start.position.y,  oppo.start.position.y,  start.y ) %>%
  gather( YPosition,  Y ) 
head(point1)
point1_df <- merge(Xpoint1_df, Ypoint1_df, by=0)
```
...
hull_df <- point1_df %>%
  select(X,Y)
c.hull <- chull(hull_df)
c.hull2 <- c(c.hull)

```
```{r}
hull2_df <- as.data.frame(cbind(1,hull_df[c.hull2 ,]$X,hull_df[c.hull2 ,]$Y))
colnames(hull2_df) <- c('ID','X','Y')


```

```{r}
ggplot(hull2_df, aes(x=X, y=Y)) + geom_polygon() 
```
```{r}
# library(depmixS4)
