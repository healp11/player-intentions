...
fed16_df <- fed16_df %>%
  ...
  mutate (is.rally = ifelse(final.shot >= 3, TRUE, FALSE)) %>%
  mutate(end.rally = ifelse(shot == final.shot & is.rally == "TRUE",1,0)) %>%
  mutate(start.rally = ifelse(shot == 3 & is.rally == "TRUE",1,0)) %>%
  mutate(ral.num = sum(start.rally))
...
```{r}
#New Variables for player positioning

fed16_df <- fed16_df %>%
  mutate(avg.player.speed.diff = NA) %>%
  mutate(peak.player.speed.diff = NA) %>%
  mutate(avg.player.speed.ratio = NA) %>%
  mutate(peak.player.speed.ratio = NA) %>%
  mutate(avg.player.acceleration.diff = NA) %>%
  mutate(peak.player.acceleration.diff = NA) %>%
  mutate(avg.player.acceleration.ratio = NA) %>%
  mutate(peak.player.acceleration.ratio = NA) %>%
  mutate(oppo.start.position.x = NA) %>%
  mutate(oppo.start.position.y = NA) %>%
  mutate(oppo.end.position.x = NA) %>%
  mutate(oppo.end.position.y = NA) %>%
  mutate(p.start.position.x = NA) %>%
  mutate(p.start.position.y = NA) %>%
  mutate(p.end.position.x = NA) %>%
  mutate(p.end.position.y = NA) %>%
  mutate(oppo.distance = NA) %>%
  mutate(p.distance = NA) %>%
  mutate(p.peak.speed = NA) %>%
  mutate(p.avg..speed = NA) %>%
  mutate(p.avg.acceleration = NA) %>%
  mutate(p.peak.acceleration = NA)
  mutate(oppo.peak.speed = NA) %>%
  mutate(oppo.avg.speed = NA) %>%
  mutate(oppo.avg.acceleration = NA) %>%
  mutate(oppo.peak.acceleration = NA) 
...
for (i in 2:nrow(fed_df)) {
  ...
  fed_df$avg.player.speed.diff[i]=fed_df$avg.server.speed[i]-fed_df$avg.server.speed[i]
  fed_df$peak.player.speed.diff[i]=fed_df$peak.server.speed[i]-fed_df$peak.server.speed[i-1]
  fed_df$avg.player.acceleration.diff[i]=fed_df$avg.server.acceleration[i]-fed_df$avg.server.acceleration[i]
  fed_df$peak.player.acceleration.diff[i]=fed_df$peak.server.acceleration[i]-fed_df$peak.server.acceleration[i-1]
  #speed difference
  
  fed_df$oppo.start.position.x[i]=fed_df$start.x[i-1]
  fed_df$oppo.start.position.y[i]=fed_df$start.y[i-1]
  fed_df$oppo.end.position.x[i]=fed_df$start.z[i-1]
  fed_df$oppo.end.position.y[i]=fed_df$speed1[i-1]
  fed_df$p.start.position.x[i]=fed_df$start.x[i-1]
  fed_df$p.start.position.y[i]=fed_df$start.y[i-1]
  fed_df$p.end.position.x[i]=fed_df$start.z[i-1]
  fed_df$p.end.position.y[i]=fed_df$speed1[i-1]
...
fed16_df <- fed16_df %>%
  mutate(player.avg.speed.ratio = speed.start/oppo.speed) %>% #Player average speed ratio
   mutate(player.peak.speed.ratio = speed.start/oppo.speed) %>% #Player average speed ratio
  mutate(side.dist = 4.115 - abs(oppo.projected.ballmark.y)) %>% #distance of oppo.projected.ballmark from sideline
  mutate(base.dist = 11.89 - abs(oppo.projected.ballmark.x)) %>% #distance of oppo.projected.ballmark from baseline
  mutate(short.dist = min(side.dist, base.dist)) #shortest distance from any line
```
