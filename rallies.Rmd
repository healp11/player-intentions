...
```{r}
fed16_df <- fed16_df %>%
  mutate(end.rally = ifelse(shot == final.shot & is.rally == "TRUE",TRUE,FALSE)) %>%
  mutate(start.rally = ifelse(shot == 3 & is.rally == "TRUE",TRUE,FALSE)) 
...
for (i in 1:nrow(fed16_df)) {
  ...
  fed16_df$p.movement.angle.1[i] = ifelse(p.movement.angle.1 < 0 & -(a1-a2)<0,180+p.movement.angle.1,
                                          ifelse(p.movement.angle.1 < 0 & -(b1-b2)<0,360+p.movement.angle.1,
                                                 ifelse(p.movement.angle.1 > 0 & -(a1-a2)<0,180+p.movement.angle.1,p.movement.angle.1)))
