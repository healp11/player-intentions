...
An increases oppo.start.position.x decreases the probability of hitting a winner. As Berdych starts further away from the net Federer is more likely to hit a winner. 
...
Unfortunately there were some issues with my models. 
...
The three state model would be useful for identifying the probability of the player hitting a winner or an error from an attacking, returning or a defensive state. A larger probability of hitting a winner from a defensive state compared to other players, may be an indicator of player risk taking. 
...
We can take the implied states from the Depmix Model probs and overlay them into the animation to visualise when the player is in an attacking state. In this case we are looking at the longest rally between Federer and Berdych in 2016. Federer will get larger to indicate when he is an attacking state.
...
Confirming choices through visualisation of all variables described in glossary
